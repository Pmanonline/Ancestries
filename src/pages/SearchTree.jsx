  ...
  const ResultCard = ({ result }) => {
    ...
    const InfoItem = ({ icon, label, value, onClick }) => (
      <div className="flex items-center space-x-2 text-sm">
        {React.cloneElement(icon, { size: 16, className: "text-gray-500" })}
        <div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col h-full">
        <div className="relative h-40 overflow-hidden">
          {result.image ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-blue-400 text-white text-3xl font-bold">
        ...
        <div className="p-3 flex-grow">
          <div className="grid grid-cols-2 gap-2">
            <InfoItem icon={<User />} label="Gender" value={result.gender} />
            <InfoItem
              icon={<Calendar />}
              label="Birth"
            ...
            <InfoItem
              icon={<MapPin />}
              label="Location"
              value={result.placesLived}
              onClick={handleLocationClick}
            />
            <InfoItem
              icon={<UserCircle />}
              label="Role"
              value={result.role === "main" ? "Tree Creator" : result.role}
            />
          </div>
        ...
        {LoggedId !== result.userId && (
          <div className="p-2 bg-blue-50">
            <button
              ...
              className="w-full bg-blue-500 text-white hover:bg-blue-600 py-1.5 px-3 rounded text-sm flex items-center justify-center space-x-1 transition duration-200"
            >
              <span>
                {result.role !== "main"
