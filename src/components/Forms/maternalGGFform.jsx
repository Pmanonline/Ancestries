...
const MaternalGGFform = ({ initialState = {}, isEdit = false }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEdit && initialState._id) {
      ...
      dispatch(
        createFamilyMember({
          memberType: "maternalGrtGrandFather",
  ...
  useEffect(() => {
    if (success) {
      ...
      setTimeout(() => navigate(`/layout/maternalGGMother-form/${userId}`));
    }
          ...
          >
            ...
            {isEdit ? (
              ...
              <div className="w-full text-end items-end flex justify-end">
                <Link
                  to={`/layout/maternalGGMother-form/${userId}`}
...
export default MaternalGGFform;
