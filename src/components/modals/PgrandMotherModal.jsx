...
export const PGMModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !PGMData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", PGMData._id);
          ...
          {PGMData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {PGMData.image ? (
                  <img
                    src={`${backendURL}/${PGMData.image}?${Date.now()}`}
                    alt={PGMData.firstName}
                ...
                {PGMData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {PGMData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{PGMData.Lstatus}</p>
