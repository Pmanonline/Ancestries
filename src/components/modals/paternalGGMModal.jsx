...
export const PGGMModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !PGGMData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", PGGMData._id);
          ...
          {PGGMData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {PGGMData.image ? (
                  <img
                    src={`${backendURL}/${PGGMData.image}?${Date.now()}`}
                    alt={PGGMData.firstName}
                ...
                {PGGMData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {PGGMData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{PGGMData.Lstatus}</p>
