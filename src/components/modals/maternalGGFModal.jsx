...
export const MGGFModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !MGGFData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", MGGFData._id);
          ...
          {MGGFData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {MGGFData.image ? (
                  <img
                    src={`${backendURL}/${MGGFData.image}?${Date.now()}`}
                    alt={MGGFData.firstName}
                ...
                {MGGFData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {MGGFData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{MGGFData.Lstatus}</p>
