...
// export const PGFModal = React.forwardRef(({ userId }, ref3) => {
//   const [open, setOpen] = React.useState(false);
//   const dispatch = useDispatch();
//   const PGFData = useSelector((state) => state.person.PGF);
...
//                 <Button
//                   href={`/layout/paternalGrandfather-form/${userId}`}
...
export const PGFModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !PGFData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", PGFData._id);
          ...
          {PGFData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {PGFData.image ? (
                  <img
                    src={`${backendURL}/${PGFData.image}?${Date.now()}`}
                    alt={PGFData.firstName}
                ...
                {PGFData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {PGFData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{PGFData.Lstatus}</p>
