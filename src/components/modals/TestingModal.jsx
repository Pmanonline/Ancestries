...
export const TESTING = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !PGGFData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", PGGFData._id);
          ...
          {PGGFData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {PGGFData.image ? (
                  <img
                    src={`${backendURL}/${PGGFData.image}?${Date.now()}`}
                ...
                {PGGFData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {PGGFData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{PGGFData.Lstatus}</p>
