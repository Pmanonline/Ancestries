...
// import MaternalGGMform from "../Forms/maternalGGMform";
...
//   );
//   const MGGMData = useSelector((state) => state.person.MGGM);
//   console.log("paternal great grand fathers details.", MGGMData);
...
// export const MGGFMModal = React.forwardRef(({ userId }, ref3) => {
//   const [open, setOpen] = React.useState(false);
//   const dispatch = useDispatch();
//   const MGGMData = useSelector((state) => state.person.MGGM);
...
//   const handleDelete = () => {
//     if (MGGMData._id) {
//       dispatch(deletePerson(MGGMData._id));
//       setOpen(false);
//     } else {
//       console.error("Invalid person ID");
//     }
//   };
//   const yearOfBirth = MGGMData?.DOB ? MGGMData.DOB.split("-")[0] : "Unknown";
//   const yearOfDeath = MGGMData?.yearDeceased
//     ? MGGMData.yearDeceased.split("-")[0]
//     : "Unknown";
...
//                   {MGGMData.firstName} {MGGMData.lastName}
//                 </h5>
//                 <span className="text-sm text-gray-500 dark:text-gray-400">
//                   {MGGMData.Lstatus !== "Deceased" ? (
//                     <>
//                       Born on:
...
export const MGGFMModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !MGGMData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", MGGMData._id);
          ...
          {MGGMData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {MGGMData.image ? (
                  <img
                    src={`${backendURL}/${MGGMData.image}?${Date.now()}`}
                    alt={MGGMData.firstName}
                ...
                {MGGMData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {MGGMData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{MGGMData.Lstatus}</p>
