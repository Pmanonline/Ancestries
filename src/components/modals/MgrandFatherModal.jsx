...
//   const MGFData = useSelector((state) => state.person.MGF);
...
//             <Spinner />
//           ) : (
//             <MaternalGrandfatherForm
//               initialState={MGFData}
//               isEdit={true}
...
// export const MGFModal = React.forwardRef(({ userId }, ref3) => {
//   const [open, setOpen] = React.useState(false);
//   const dispatch = useDispatch();
//   const MGFData = useSelector((state) => state.person.MGF);
...
//   const handleDelete = () => {
//     if (MGFData._id) {
//       dispatch(deletePerson(MGFData._id));
//       setOpen(false);
//     } else {
...
//       >
//         <Box sx={{ ...style, width: 400 }}>
//           {MGFData && Object.keys(MGFData).length > 0 ? (
...
export const MGFModal = React.forwardRef(({ userId }, ref3) => {
  ...
  const handleUpload = async () => {
    if (!selectedFile || !MGFData?._id) return;

    const formData = new FormData();
    formData.append("image", selectedFile);
    formData.append("personId", MGFData._id);
          ...
          {MGFData ? (
            <div className="w-full max-w-sm bg-white  rounded-lg    relative">
              ...
              <div className="flex flex-col items-center">
                {MGFData.image ? (
                  <img
                    src={`${backendURL}/${MGFData.image}?${Date.now()}`}
                    alt={MGFData.firstName}
                ...
                {MGFData.placesLived && (
                  <p className="text-sm text-gray-500 mb-1">
                    Place lived: {MGFData.placesLived}
                  </p>
                )}
                <p className="text-sm text-gray-500 mb-4">{MGFData.Lstatus}</p>
